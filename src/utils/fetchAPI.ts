...
export const fetchAPI = async (
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Error: ${errorData.message || response.statusText}. ${method} request to ${full_url} failed.`);
        }

        const contentType = response.headers.get('content-type');

        return contentType?.includes('application/json') ? response.json() : response; // check if the response is json before parsing
    } catch (error) {
