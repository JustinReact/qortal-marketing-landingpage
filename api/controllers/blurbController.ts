...
export const handleAddBlurb = async (
  ...
  try {
    // Update the blurb in Firestore for the subscriber
    await addBlurbByEmail({ email, blurb });
  } catch (error) {
    console.error("[Error while adding a blurb] Failed to add blurb.", error);
    res
      .status(500)
      .json({ error: "Failed to add blurb. Please try again later." });
    return;
  }
...
export const handleGetAllBlurbs = async (
  ...
  try {
    // Fetch all blurbs from Firestore
    const data = await getAllBlurbs();
    res.status(200).json(data);
  } catch (error) {
    console.error(
      "[Error while getting all blurbs] Failed in handleGetAllBlurbs.",
      error
    );
    res
      .status(500)
      .json({ error: "Failed to get blurbs. Please try again later" });
  }
};
...
export const handleGetAllSubscribers = async (
  ...
  try {
    ...
    console.error(
      "[Error while getting all subscribers] Failed in handleGetAllSubscribers.",
      error
    );
    res
      .status(500)
      .json({ error: "Failed to get subscribers. Please try again later" });
  }
};
